															...
															if (!empty($_POST['startD'])&&!empty($_POST['endD']))
															{
																...
																$eDate = $_POST['endD'];
																			 ...
																			 $sql = "SELECT* from transaction t , account a ,client c where t.acc_Number = a.acc_Number AND a.cli_ID = c.cli_ID 
													                                AND c.cli_IDNo = '$IDNUM' AND t.trans_Date >= DATE('$sDate') AND t.trans_Date <= DATE('$eDate') ";
											...
											if (isset($_POST['statement'])&&isset($_SESSION['ID'])&&$_SESSION['userLevel'] == "yes" && isset($_SESSION['bank']))
											{
												...
												if (!empty($_POST['startD'])&&!empty($_POST['endD']))
												{
													...
													if ($sDate>$eDate || $sDate > $currentD)
													{
														...
														$_SESSION['message'] = "Start  date cannot be greater then end date";
                                                        $_SESSION['msg_type'] = "danger";
														echo "<script>location.replace('statement.php');</script>";
														exit();
													}
													else
													{
												...
												else
												{
													...
													$_SESSION['message'] = "Select Start date and End date first for Statement";
                                                    $_SESSION['msg_type'] = "danger";
													echo "<script>location.replace('statement.php');</script>";
													exit();
												}
